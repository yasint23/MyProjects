...
locals {
  github-repo = "https://github.com/yasint23/phonebook1.git"
  github-file-url = "https://raw.githubusercontent.com/yasint23/phonebook1/master/"
}
...
resource "aws_instance" "docker-machine-managers" {
  ...
  tags = {
    Name = "yasin-Docker-Swarm-Manager-${count.index + 1}"
  }
...
resource "aws_instance" "docker-machine-workers" {
  ...
  tags = {
    Name = "yasin-Docker-Swarm-Worker-${count.index + 1}"
  }
  depends_on = [aws_instance.docker-machine-leader-manager]
}
...
resource "aws_security_group" "tf-docker-sec-gr" {
  name = "yasin-docker-swarm-sec-gr"
  tags = {
    Name = "swarm-sec-gr"
  }
  dynamic "ingress" {
